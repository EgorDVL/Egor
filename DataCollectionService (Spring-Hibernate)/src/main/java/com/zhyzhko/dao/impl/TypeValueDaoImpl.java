...
public class TypeValueDaoImpl implements TypeValueDao {
    ...
    @Override
    public List<TypeValue> getAllValueByTypeId(int typeId) {
        Session session = sessionFactory.getCurrentSession();
        return session
                .createQuery("from TypeValue where typeId = :ID", TypeValue.class)
    ...
    public boolean deleteTypeValue(int typeValueId) {
        Session session = sessionFactory.getCurrentSession();
        return session
                .createQuery("delete TypeValue where id = :ID")
                .setParameter("ID", typeValueId)
                .executeUpdate() > 0;
    }
}